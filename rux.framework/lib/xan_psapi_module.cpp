...
namespace rux
{
	namespace system
	{
		...
		BOOL psapi_module::EnumProcesses( DWORD* pProcessIds , DWORD cb , DWORD* pBytesReturned )
		{
			load();
			return _EnumProcesses( pProcessIds , cb , pBytesReturned );
		};
		BOOL psapi_module::EnumProcessModules( HANDLE hProcess , HMODULE* lphModule , DWORD cb , LPDWORD lpcbNeeded )
		{
			load();
			return _EnumProcessModules( hProcess , lphModule , cb , lpcbNeeded );
		};
		DWORD psapi_module::GetModuleBaseNameA( HANDLE hProcess , HMODULE hModule , char* lpBaseName , DWORD nSize )
		{
			load();
			return _GetModuleBaseNameA( hProcess , hModule , lpBaseName , nSize );
		};
		DWORD psapi_module::GetModuleFileNameExA( HANDLE hProcess , HMODULE hModule , char* lpFilename , DWORD nSize )
		{
			load();
			return _GetModuleFileNameExA( hProcess , hModule , lpFilename , nSize );
		};
		DWORD psapi_module::GetProcessImageFileNameA( HANDLE hProcess , char* lpImageFileName , DWORD nSize )
		{
			load();
