...
namespace rux
{
	namespace gui
	{
		...
		{			
			...
			dll_internal ::rux::gui::engine::schedule* _gui_schedule_processing = 0;
			...
			{	
				...
				if( ::rux::gui::engine::_is_pump_message == 1 )
				{
                  ...
                  ::rux::gui::engine::_gui_schedule_processing = schedule;
									fdispatcher(schedule->_param);
                  ::rux::gui::engine::_gui_schedule_processing = 0;
			...
			dll_internal void remove_schedule( size_t schedule_index )
			{
				...
				if(::rux::engine::_globals->_gui_globals->_gui_schedule->Count() > schedule_index)
				{
					...
					else if(::rux::gui::engine::_gui_schedule_processing == schedule)
			...
			dll_internal void dispatch_create_window( void* param )
			{
					...
					&& param )
				{
					...
					::rux::log::WriteTrace("dispatch_create_window, %pp, x=%d, y=%d, width=%d, height=%d", window, window->_left
						, window->_top, window->_width, window->_height);
					MWMHints mwmhints;
					Atom prop;
