...
void main() {
  ...
  y = texture( yuyv_tex , vec2( nx , ny ) ).r;
  if( int( nx ) % 2 == 1 )
  {
  v = texture( yuyv_tex , vec2( nx , ny ) ).a;
  u = texture( yuyv_tex , vec2( nx , ny ) + vec2( 1.0 , 0.0 ) ).a;
  }
  else
  {
  v = texture( yuyv_tex , vec2( nx , ny ) - vec2( 1.0 , 0.0 ) ).a;
  u = texture( yuyv_tex , vec2( nx , ny ) ).a;
  }
  y=1.1643*(y-0.0625);
  u=u-0.5;
  v=v-0.5;
