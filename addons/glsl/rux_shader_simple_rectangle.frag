...
bool check_colors_offset( int colors_offset_index )
{
	...
	if( percent < get_colors_offset( colors_offset_index ) )
		gl_FragColor = compute_color( percent , get_colors_offset( colors_offset_index ) , get_common( colors_offset_index - 1 ) , get_common( colors_offset_index ) );
	else if( int( get_common( 7 ).r ) == colors_offset_index + 1 )
		gl_FragColor = vec4( get_common( colors_offset_index ).r , get_common( colors_offset_index ).g , get_common( colors_offset_index ).b , get_common( colors_offset_index ).a * get_common( 12 ).a );
	else
		return false;
	return true;
}
void main()
{
		...
		|| get_common( 12 ).r > frag_coord_y || frag_coord_y > get_common( 10 ).g - get_common( 12 ).r )
	{
		if( get_common( 11 ).a > 0.0 )
			gl_FragColor = vec4( get_common( 11 ).r , get_common( 11 ).g , get_common( 11 ).b
	...
	else
	{
		...
		else if( int( get_common( 7 ).r ) == 1 )
		{
			if( get_common( 0 ).a != 0.0 )
				gl_FragColor = vec4( get_common( 0 ).r , get_common( 0 ).g , get_common( 0 ).b , get_common( 0 ).a * get_common( 12 ).a );
			else
				discard;
		}
		else
